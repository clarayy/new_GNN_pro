...
class GraphSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, G_list, features='default', normalize=True, assign_feat='default', max_num_nodes=0):
        ...
        if max_num_nodes == 0:     #要么是固定值，要么随着G的变化而变，在这里是最开始设定的值100
            self.max_num_nodes = max([G.number_of_nodes() for G in G_list])
        else:
            self.max_num_nodes = max_num_nodes

        #if features == 'default':
        self.feat_dim = util.node_dict(G_list[0])[0]['feat'].shape[0] #当G_list为0时，即该分类中没有图片时报错，因此直接改为max_num_nodes
        #self.feat_dim = max_num_nodes
        for G in G_list:
            ...
            self.classlabel_all.append(G.graph['classlabel'])###
            self.center_all.append(G.graph['center'])###
            self.unbet_all.append(G.graph['unbet'])  ###
            self.discen_all.append(G.graph['discen'])  ###
            self.dynage_all.append(G.graph['dynage'])  ###
            ...
            if features == 'default':
                ...
                self.feature_all.append(np.identity(self.max_num_nodes))#创建对角为1的方阵
        ...
        #print(self.assign_feat_all[0])
        self.assign_feat_dim = self.assign_feat_all[0].shape[1]   ##IndexError: list index out of range 当该分类的图为0张时报错
        #self.assign_feat_dim = max_num_nodes
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        return {'adj':adj_padded,
                ...
                'classlabel':self.classlabel_all[idx],###
                'center':self.center_all[idx],    ###
                'unbet': self.unbet_all[idx],     ###
                'discen': self.discen_all[idx],
                'dynage': self.dynage_all[idx],
                'num_nodes': num_nodes,
