# 从本程序中train改变而来，也就是说在之前实验程序的基础上进行分类的设计，但是没成功
# 进阶版是classtrain1.py
# prepare_data_pro将数据集分为三类，训练/验证/测试
# --coding:utf-8--
import matplotlib
...
def evaluate(dataset, model, args, name='Validation', max_num_examples=None):
    ...
    for batch_idx, data in enumerate(dataset):
        ...
        # label_class = data['label'].numpy()
        #
        # for x in label_class:
        #     new_label.append(dolphins_class[x])
        # new_label = Variable(torch.tensor(new_label).long()).to(device)
    ...
    read_dic = np.load("BA200_short_path.npy", allow_pickle=True).item()
    # print(read_dic[2][3])
    distance = []
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, data in enumerate(dataset):
            ...
            if not args.method == 'soft-assign' or not args.linkpred:
                # print(label.shape)
                # print(ypred.shape)
                loss = model.loss(ypred, new_label)
            else:
                loss = model.loss(ypred, new_label, adj, batch_num_nodes)
            loss.backward()
        ...
        result, train_labels, train_preds, train_jordancenter, train_unbet, train_discen, train_dynage = evaluate(
            dataset, model, args, name='Train', max_num_examples=100)
        train_accs.append(result['acc'])
        train_epochs.append(epoch)
        if val_dataset is not None:
            val_result, val_labels, val_preds, val_jordancenter, val_unbet, val_discen, val_dynage = evaluate(
                val_dataset, model, args, name='Validation')
        ...
        if test_dataset is not None:
            test_result, test_labels, test_preds, test_jordan_center, test_unbet, test_discen, test_dynage = evaluate(
                test_dataset, model, args, name='Test')
...
def benchmark_task_val(args, writer=None, feat='node-label'):
    ...
    for i in range(10):
        train_dataset,train_c0, train_c1, train_c2, train_c3,train_c4, val_dataset, max_num_nodes, input_dim, assign_input_dim = \
        ...
        if args.method == 'soft-assign':
            ...
            print('Method: base')
            model1 = encoders.GcnEncoderGraph(
                input_dim, args.hidden_dim, args.output_dim, args.num_classes,
                args.num_gc_layers, bn=args.bn, dropout=args.dropout, args=args).to(device)  # cuda()改
            # print(model)
        _, val_accs = train(train_dataset, model1, args, val_dataset=val_dataset, test_dataset=None,
                            writer=writer)
        # newmodel = encoders.GcnEncoderGraph(
        #     input_dim, args.hidden_dim, args.output_dim, args.num_classes,
        #     args.num_gc_layers, bn=args.bn, dropout=args.dropout, args=args).to(device)
        # new_, new_val_accs = train(train_dataset, newmodel, args, val_dataset=val_dataset,
...
def main():
    ...
    if os.path.isdir(path):
        ...
        shutil.rmtree(path)  # 递归的删除文件，该文件夹下的所有内容
