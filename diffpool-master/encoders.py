...
class GraphConv(nn.Module):
    def __init__(self, input_dim, output_dim, add_self=False, normalize_embedding=False,
        ...
        self.weight = nn.Parameter(torch.FloatTensor(input_dim, output_dim).to(device))#.cuda()改为.to(device)
        if bias:
...
class GcnEncoderGraph(nn.Module):
    def __init__(self, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        for m in self.modules():
            if isinstance(m, GraphConv):    #判断m是否是类型 GraphConv
                m.weight.data = init.xavier_uniform(m.weight.data, gain=nn.init.calculate_gain('relu'))#xavier初始化方法中服从均匀分布
    ...
    def construct_mask(self, max_nodes, batch_num_nodes): 
        ...
        return out_tensor.unsqueeze(2).to(device)#cuda改

    def apply_bn(self, x):
        ''' Batch normalization of 3D tensor x
        '''
        bn_module = nn.BatchNorm1d(x.size()[1]).to(device)#cuda改
        return bn_module(x)
